...
def get_stock_list(context):
    ...
    # ↑距离上一个工作日300天前 
    # 获取证券list  ['000001.XSHE',..., '000002.XSHE']
    stock_list = list(get_all_securities(date=check_date).index)
    # pb符合要求的股票，按照流通市值升序排列，取前1000名
    q = query(
        valuation.code # 搜索该字段
    ).filter(
        valuation.code.in_(stock_list),# 从stock_list列表里面
        valuation.pb_ratio.between(g.pb_min, g.pb_max) # 市净率0.01 到 30之间
    ).order_by(
        valuation.circulating_market_cap.asc() # 流通市值 升序
    ).limit(
        1000
    )
...
def get_bull_bear_signal_minute():
    ...
    if g.is_bull:
        if close_data[-1] * (1 + g.threshold) <= ma_old:
            # 今天的收盘跌出，平均值阈值以下
            g.is_bull = False # 设置为熊市
    else:
        if close_data[-1] > ma_old * (1 + g.threshold):
            # 今天的收盘高出，平均值阈值以上
            g.is_bull = True # 设置为牛市
# 股票精选
...
def my_buy(context):
    if context.current_dt.month == 4: # 4月不持仓
        return
    if g.not_hold: # 不持仓信号
        return 
    # 避免清仓的股票马上买入，排除不稳定股票
