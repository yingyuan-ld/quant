...
def get_stock_list(context):
    ...
    limit_info_raw = check_limit(stock_list)
    limit_info = {s: limit_info_raw.get(s, 0) for s in stock_list}
    stock_list = [s for s in stock_list if not (
        halt.get(s, True) or
        limit_info.get(s, 0) == 1 or
        limit_info.get(s, 0) == -1 or
...
def get_bull_bear_signal_minute(context):
    ...
    try:
        close_data = get_history(g.MA[1], '1d', 'close', security_list=[g.MA[0]],
                                fq=None, include=False, is_dict=True)
    except Exception:
        return
    ...
    sec_data = close_data.get(g.MA[0]) if isinstance(close_data, dict) else close_data
    if sec_data is None:
        return

    if isinstance(sec_data, dict):
        close_arr = sec_data.get('close')
    elif isinstance(sec_data, np.ndarray) and sec_data.dtype.names is not None and 'close' in sec_data.dtype.names:
        close_arr = sec_data['close']
    else:
        close_arr = sec_data
...
def _get_macd_dif_dea(security):
    """返回指定股票最新一根 K 线的 (dif, dea)，数据不足或异常时返回 (None, None)。"""
    try:
        close_data = get_history(100, '1d', 'close', security_list=[security],
                                 include=True, is_dict=True)
    except Exception:
        return None, None

    sec_data = close_data.get(security) if isinstance(close_data, dict) else close_data
    if isinstance(sec_data, dict):
        close_arr = sec_data.get('close')
    else:
        close_arr = sec_data
    ...
    # MACD(12, 26, 9) 至少需要 26 + 9 - 1 = 34 根数据
    if arr.size < 35:
        return None, None

    try:
        dif_data, dea_data, _ = get_MACD(arr, 12, 26, 9)
    except Exception:
        return None, None

    if dif_data is None or dea_data is None or len(dif_data) == 0 or len(dea_data) == 0:
        return None, None
    ...
    try:
        ...
        return None, None
    if math.isnan(dif) or math.isnan(dea):
        return None, None
...
def get_stock_rank_m_m(context, data):
    ...
    g.chosen_stock_list = []  # 先清空，避免中途 return 残留旧列表
    if len(stock_list) == 0:
        return
    prev_date = get_trading_day(-1)
    date_str = prev_date.strftime('%Y-%m-%d') if hasattr(prev_date, 'strftime') else str(prev_date)
    try:
        df = get_fundamentals(stock_list, 'valuation',
    ...
    def _extract_symbol_payload(container, symbol):
        ...
        if arr.size == 0:
            return None
        if arr.dtype.names is not None and symbol in arr.dtype.names:
            return arr[symbol]
        return arr

    vol_map = {}
    for s in stock_list:
        sym_payload = _extract_symbol_payload(vol_1m, s)
        vol_arr = _extract_field(sym_payload, 'volume')
        if vol_arr is None:
            vol_arr = _extract_field(sym_payload, 'vol')
        vol_arr = np.asarray(vol_arr) if vol_arr is not None else np.array([0.0])
        if vol_arr.size == 0:
            vol_map[s] = 0.0
        else:
            vol_map[s] = float(np.nansum(vol_arr.astype(float)))
    ...
    top_list = list(sort_res.index)[:g.rank_top_n]
    # MACD 金叉筛选：只保留 dif > dea（处于金叉上升态）的股票
    macd_list = []
    for s in top_list:
        dif, dea = _get_macd_dif_dea(s)
        if dif is not None and dea is not None and dif > dea:
            macd_list.append(s)
    g.chosen_stock_list = macd_list
...
def my_adjust_position(context, data):
    # 牛熊仓位控制：熊市只使用 bear_pct 比例资金，牛市满仓
    ...
    for stock in list(context.portfolio.positions.keys()):
        ...
        try:
            ...
            limit_status = 0
        dif, dea = _get_macd_dif_dea(stock)

        # 1) MACD 死叉（dea > dif）立刻清仓，不检查涨停
        macd_dead = (dif is not None and dea is not None and dea > dif)
        if macd_dead:
            close_position(context, stock)
            continue

        # 2) 轮动卖出：不在候选列表且未涨停 → 清仓
        sell_1 = limit_status != 1
        sell_2 = stock not in g.chosen_stock_list
        if sell_2 and sell_1:
            close_position(context, stock)
            continue

        # 3) 再平衡：单票占比超过上限 → 减仓到标准仓位
...
def my_trade(context, data):
    ...
    # 熊市且不参与熊市（bear_pos=False）→ 清仓空仓
    if g.choose_time_signal and (not g.is_bull) and (not g.bear_pos):
        ...
        # 前10只中若无符合 MACD 金叉的股票，则不买入
        g.not_hold = (len(g.chosen_stock_list) == 0)
...
def my_buy(context, data):
    ...
    buy_cash = free_value / g.stock_nums
    position_count = len([p for p in context.portfolio.positions.values() if getattr(p, 'amount', 0) > 0])
